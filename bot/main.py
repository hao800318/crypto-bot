...
def _trim_rebase(df):
    """
    裁掉合約改版（rebase）前的歷史資料。

    OKX 某些合約在改版後，舊 K 線的價格與現行合約完全不在同一量級
    （如 MUU 舊合約最高 777，新合約最高 46），但 API 仍把兩段資料合在一起回傳。

    做法：掃描相鄰收盤價的比率，若任意相鄰兩根的比率 >5× 或 <0.2×
    （即單根 K 線漲跌 >80%），視為價格斷層，只保留最後一段連續資料。
    """
    if len(df) < 2:
        return df
    ...
    if last_break == -1:
        return df   # 無斷層，全段使用
    trimmed = df.iloc[last_break:].reset_index(drop=True)
...
def _find_swing_df(df, is_bull, current_price, max_dist_pct=15.0):
    ...
    先裁掉合約 rebase 前的歷史資料，再從剩餘的連續段挑最適合的視窗。
...
def fetch_range_signal(asset, tf, max_leverage=20, ref_trends=None, market_fr=0.0):
    ...
    try:
        ...
        # ── HTF 斐波那契匯流確認 ──
        # 區間邊界觸及後，用高一級時框（1H when on 15M）的 Fib 回撤位確認支撐/壓力區
        # 是否同時對齊大結構 Fib 水位。支撐 + HTF Fib 0.618 共鳴是最強品質的區間進場。
        # 這是評分加成，不是硬過濾，避免 Fib 計算失敗時大量流失有效的區間訊號。
        _htf_bar_r   = _TF_HTF.get(tf, "1H")   # e.g. 15m→"1H", 1h→"4H"
        _fib_lbl_r   = None
        _fib_near_r  = False
        _fib_bonus_r = 0
        _fib_label_r = ""
        try:
            _m_url = (f"{BASE_URL}/api/v5/market/candles"
                      f"?instId={asset}&bar={_htf_bar_r}&limit=100")
            ...
            if _m_res.get('code') == '0' and len(_m_res.get('data', [])) >= 50:
                ...
                if _fib_near_r and _fib_lbl_r in ('0.382', '0.618'):
                    _fib_bonus_r = 10
                    _fib_label_r = f"📐HTF{_htf_bar_r}Fib{_fib_lbl_r}匯流"
                    ...
                    _fib_bonus_r = 2
                    _fib_label_r = f"📐HTF{_htf_bar_r}Fib{_fib_lbl_r}附近"
        except Exception:
            _fib_bonus_r = 0
        if _fib_bonus_r:
            raw      += _fib_bonus_r
...
def fetch_divergence_signal(asset, tf, max_leverage=20, ref_trends=None, market_fr=0.0):
    """
    量價背離轉折訊號（ADX 20-36，趨勢末段捕捉反轉）。

    觸發路徑 A — 經典背離（需兩個擺動點）：
      頂背離（做空）：近期更高高點，但 RSI 創低 + 量縮 + 前一棒上影線或吞沒
      底背離（做多）：近期更低低點，但 RSI 創高 + 量縮 + 前一棒下影線或吞沒

    觸發路徑 B — HTF Fib 超賣/超買（三條件）：
      條件一（1H）：價格打到 HTF Fib 0.618 / 0.786 回撤位
      條件二（同TF）：RSI < 30（超賣做多） / RSI > 70（超買做空）
      條件三（同TF）：前一棒出現長下影線 / 釘頭錘 / 多頭吞沒（做多方向）
    ...
    try:
        ...
        # ── HTF K 線（1H when on 15M）：用於 Fib 結構定位 ──
        # 路徑 A（背離）+ 路徑 B（超賣）都需要 HTF Fib 確認進場區間有大結構依據。
        _htf_bar_d = _TF_HTF.get(tf, "4H")
        df_htf_d   = None
        try:
            _htf_url_d = (f"{BASE_URL}/api/v5/market/candles"
                          f"?instId={asset}&bar={_htf_bar_d}&limit=100")
            _htf_res_d = requests.get(_htf_url_d, timeout=2.0).json()
            if _htf_res_d.get('code') == '0' and len(_htf_res_d.get('data', [])) >= 50:
                df_htf_d = pd.DataFrame(_htf_res_d['data'],
                                        columns=['ts','open','high','low','close',
                                                 'vol','volCcy','volCcyQuote','state'])
                for _hc in ['open','high','low','close','vol']:
                    df_htf_d[_hc] = df_htf_d[_hc].astype(float)
                df_htf_d = df_htf_d.iloc[::-1].reset_index(drop=True)
        except Exception:
            df_htf_d = None
        ...
        if _rt_d == "bear" and direction == "多":
            return None   # 底背離/超賣多單 + 鏈幣空頭 → 大跌趨勢中假反彈機率高
        if _rt_d == "bull" and direction == "空":
            return None   # 頂背離/超買空單 + 鏈幣多頭 → 強多趨勢中假頂機率高

        # ④ 前一根 K 棒需有拒絕信號：影線 OR 吞沒型態，任一成立即可
        # 用戶策略條件三：「長下影線（針）OR 陽線吞沒」→ 此處擴展為雙路徑
        ...
        # 路徑④-A：傳統影線拒絕（原有邏輯）
        _wick_ok = (
            (direction == "空" and upper_wk >= max(body, min_body) * 0.8) or
            (direction == "多" and lower_wk >= max(body, min_body) * 0.8)
        )
        # 路徑④-B：K 線型態吞沒 / 釘頭錘（新增）
        _candle_pat_d, _candle_bonus_d = detect_candle_pattern(df, direction, n_bars=3)
        _pattern_ok = _candle_bonus_d > 0

        if not _wick_ok and not _pattern_ok:
            return None   # 影線不足且無吞沒/釘頭錘，拒絕
        ...
        # 使用 HTF（1H when on 15M）量測 Fib，確保大結構回撤位有意義
        # HTF df 若取得失敗則降回同時框 df（保守 fallback，不完全放棄訊號）
        _fib_df_src = df_htf_d if df_htf_d is not None else df
        ...
        raw = rsi_score + wick_score + extreme_score + _fib_bonus_d + _candle_bonus_d
...
def send_loss_analysis(chat_id):
    ...
    結構完全對齊 send_stats_report：直接 requests.post，每步 print + HTTP 狀態確認。
    """
    _url = f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendMessage"
    _cid = str(chat_id)
    print(f"🔴 send_loss_analysis 開始執行，chat_id={_cid}")
    ...
    try:
        ...
        recent  = losses[:20]
        print(f"🔴 統計資料：總={len(records)}, 敗場={len(losses)}, 取前={len(recent)}")
    except Exception as ex:
        print(f"❌ load_stats 失敗: {ex}")
        requests.post(_url, json={"chat_id": _cid,
            "text": f"⚠️ 讀取統計檔案失敗：{ex}"}, timeout=10)
        return
    ...
    try:
        ...
        lines = [f"🔴 【敗場深度分析】  {now_str} PT",
                 f"最近 {len(recent)} 筆敗場（共 {len(losses)} 筆歷史）",
                 "─────────────────────────"]
        ...
        for i, r in enumerate(recent, 1):
            ...
            try:
                pnl_s  = f"{float(r.get('pnl_pct')):+.2f}%" if r.get('pnl_pct') is not None else "—"
                hold_s = f"{float(r.get('hold_mins')):.0f}分" if r.get('hold_mins') is not None else "—"
                sld_s  = f"{float(r.get('sl_dist_pct')):.2f}%" if r.get('sl_dist_pct') is not None else "—"
                fr_v   = r.get('entry_fr') or 0
                fr_s   = f"{float(fr_v):+.4f}%" if fr_v else "—"
                adx_s  = f"{float(r.get('adx') or 0):.0f}"
                sc_s   = f"{float(r.get('score') or 0):.0f}"
            except (TypeError, ValueError):
                pnl_s = hold_s = sld_s = fr_s = "—"
                adx_s = sc_s = "?"
            vol_s = "" if r.get('vol_confirmed', True) else " 無量"
            ts    = r.get('timestamp', 0)
            dt_s  = datetime.datetime.fromtimestamp(ts, tz=la_tz).strftime('%m/%d %H:%M') if ts else "—"
            lines.append(f"{i}. {asset}{dir_} {tf} {st}  {et}  {pnl_s}")
            lines.append(f"   ADX{adx_s} Sc{sc_s}  持{hold_s}  SL±{sld_s}  FR{fr_s}{vol_s}")
            lines.append(f"   {dt_s}")
        ...
        lines.append("失效因子統計")
        factors = [
            (sum(1 for r in recent if (r.get('hold_mins') or 999) < 15),
             "超快速反轉(<15分) → 疑似流動性獵取"),
            (sum(1 for r in recent if (r.get('sl_dist_pct') or 99) < 0.5),
             "SL過緊(<0.5%) → 建議加寬緩衝"),
            (sum(1 for r in recent if (r.get('sl_dist_pct') or 0) > 3.0),
             "SL過寬(>3%) → R:R惡化"),
            (sum(1 for r in recent if not r.get('vol_confirmed', True)),
             "量能不足 → 動能確認失效"),
            (sum(1 for r in recent if r.get('signal_type') == 'trend' and (r.get('adx') or 0) < 28),
             "趨勢ADX<28 → 趨勢動能不足"),
            (sum(1 for r in recent if (r.get('score') or 99) < 78),
             "評分<78 → 邊際訊號高風險"),
        ]
        any_factor = False
        for cnt, txt in factors:
            if cnt > 0:
                lines.append(f"  {txt}：{cnt}筆（{cnt/n*100:.0f}%）")
                any_factor = True
        if not any_factor:
            lines.append("  暫無明確規律，繼續積累樣本")

        st_cnt   = Counter(r.get('signal_type','trend') for r in recent)
        st_parts = [f"{_st_label.get(k,k)}x{v}" for k, v in st_cnt.most_common()]
        lines.append(f"策略分布：{'  '.join(st_parts)}")

        msg = "\n".join(lines)
        if len(msg) > 4000:
            msg = msg[:3990] + "\n…（訊息過長已截斷）"

        print(f"🔴 訊息組裝完成，長度={len(msg)}")
    except Exception as ex:
        print(f"❌ send_loss_analysis 組裝失敗: {ex}")
        requests.post(_url, json={"chat_id": _cid,
            "text": f"⚠️ 敗場分析組裝失敗：{ex}"}, timeout=10)
        return

    # ── Step 4: 發送（純文字，不用 parse_mode 避免 HTML 解析錯誤）──
    try:
        r4 = requests.post(_url, json={"chat_id": _cid, "text": msg}, timeout=15)
        print(f"🔴 敗場分析已發送，HTTP={r4.status_code}, ok={r4.json().get('ok')}")
    except Exception as ex:
        print(f"❌ send_loss_analysis 發送失敗: {ex}")
